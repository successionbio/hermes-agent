...
| `gateway_message_delivered` | Observer | After the platform acknowledges a successful non-streaming final gateway reply; return ignored. Dispatched through a host-owned bounded background queue, so callbacks cannot delay or break delivery. | `platform`, `chat_id`, `thread_id`, `message_id`, `success` (`true`), `is_final` (`true`), `is_ephemeral`, `session_key` | Routing and delivery identifiers only. Message content, raw SDK responses, adapters, and bot clients are never exposed. |
